...
# ip netns exec h2 python3 cors_server.py 8000 ---background

# Stream DASH from client
cd ../client
# echo Done ---set-time-wait 250
ip netns exec h1 vlc http://10.0.0.2:8000/dash/encode/manifest.mpd --extraintf=network:logger --verbose=2 --logfile=vlc_log.txt ---set-time-wait 2
