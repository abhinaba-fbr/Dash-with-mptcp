...
cd /home/abhinaba/Major/MPTCP/server/
ip netns exec s2 python3.11 -m http.server --protocol HTTP/1.1 ---background

# Change Directory to client
cd /home/abhinaba/Major/MPTCP/client/

# Run UDP clients on 'h1'
ip netns exec h1 iperf -u -c 12.0.0.2 -p 5001 -b 9.3M -t 0 ---background
...
ip netns exec h3 iperf -u -c 12.0.3.2 -p 5002 -b 9.3M -t 0 ---background
...
cd /home/abhinaba/Major/MPTCP/client/
ip netns exec h2 gpac -i http://12.0.1.2:8000/dash/BigBuckBunny/15sec/simple_manifest.mpd:gpac:algo=gbuf:start_with=max_q vout:buffer=1000:mbuffer=10000 -logs=all@info -log-file=gpac_log.log ---set-time-wait 5
