...
ip -n h2 mptcp endpoint add 192.168.0.2 dev eth2b signal
ip -n h1 mptcp endpoint add 192.168.0.1 dev eth1b subflow fullmesh
...
ip netns exec h1 ip link set dev eth1a down ---set-time 10
ip netns exec h1 ip link set dev eth1a up ---set-time 30
...
cd ../
mptcpize run ip netns exec h2 h2o -c examples/h2o/h2o.conf ---background

# Stream DASH from client
mptcpize run ip netns exec h1 gpac -gui http://10.0.0.2:8080/dash/encode/manifest.mpd ---set-time-wait 5
