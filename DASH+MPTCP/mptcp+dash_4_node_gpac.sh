...
ip -n h2 mptcp endpoint add 192.168.1.2 dev eth4b signal
ip -n h1 mptcp endpoint add 192.168.0.1 dev eth2a subflow fullmesh
...
# Test by turning off the interface (optional) checking for resilence
...
mkdir -p captures
ip netns exec h2 tshark -i eth3b -w captures/master.pcap ---background
ip netns exec h2 tshark -i eth4b -w captures/subflow.pcap ---background
...
cd ../client
mptcpize run ip netns exec h2 gpac -i http://10.0.1.2:8000/dash/BigBuckBunny/2sec/simple_manifest.mpd:gpac:algo=gbuf:start_with=min_bw aout vout:buffer=1000:mbuffer=10000 -logs=all@info -log-file=gpac_log.log ---set-time-wait 3
